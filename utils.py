...
def get_similarity_map(sm, shape):
    num_elements = sm.shape[1]
    side = int(num_elements ** 0.5)
    if side * side != num_elements:
        print(f"Warning: Tensor size {num_elements} does not allow reshaping into a square. Using alternative approach.")
        # Adjust processing for non-square reshaping
        sm = sm.view(sm.shape[0], num_elements, 1)  # Ensure this creates a 3D tensor before interpolation
        sm = torch.nn.functional.interpolate(sm.permute(0, 2, 1).unsqueeze(1), shape, mode='bilinear')
        sm = sm.squeeze(1).permute(0, 2, 1)
    else:
        sm = sm.reshape(sm.shape[0], side, side, -1).permute(0, 3, 1, 2)
        sm = torch.nn.functional.interpolate(sm, shape, mode='bilinear')
    
    # Return a 4D tensor by ensuring an extra channel dimension
    return sm.unsqueeze(1)  # Add a channel dimension: [1, 1, 224, 224]
...
def zero_clapping(similarity_maps, threshold):
    print(f"similarity_maps shape inside zero_clapping: {similarity_maps.shape}")
    
    # Ensure 4D input
    if len(similarity_maps.shape) == 5:
        similarity_maps = similarity_maps.squeeze(2)
    elif len(similarity_maps.shape) == 2:
        similarity_maps = similarity_maps.unsqueeze(0).unsqueeze(0)
    elif len(similarity_maps.shape) != 4:
        raise ValueError(f"Unexpected shape of similarity_maps: {similarity_maps.shape}")
    
    max_ps = torch.max(similarity_maps, dim=1, keepdim=True)[0]
    min_ps = torch.min(similarity_maps, dim=1, keepdim=True)[0]
    normalized_sm = (similarity_maps - min_ps) / (max_ps - min_ps)
    normalized_sm[normalized_sm < threshold] = 0
    normalized_sm[normalized_sm >= threshold] = 1
    
    print(f"normalized_sm shape after thresholding: {normalized_sm.shape}")
...
def get_train_classes(dataset, max_classes=3):
    train_classes = []
    for i, (_, _, caption) in enumerate(dataset):  # Unpack sketch, stroke_data, caption
        if isinstance(caption, torch.Tensor):
            caption = caption.numpy().tobytes().decode('latin1')  # Convert Tensor to string using 'latin1'
        elif isinstance(caption, np.ndarray):
            caption = caption.tobytes().decode('latin1') 
        caption = caption.replace('\n',' ')
        translator = str.maketrans('', '', string.punctuation)
        caption = caption.translate(translator).lower()
        words = nltk.word_tokenize(caption)
...
def collate_fn(batch):
    sketches, stroke_data, captions = zip(*batch)
    
    # Stack sketches and captions as they are (no need for padding)
    sketches = torch.stack(sketches)
    captions = list(captions)  # If captions are strings, keep them as a list
    
    # Pad stroke data so that all sequences have the same length
    stroke_data_padded = pad_sequence(stroke_data, batch_first=True, padding_value=0)
    
    return sketches, stroke_data_padded, captions